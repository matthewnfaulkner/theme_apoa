...
class apoanavbar implements \renderable {
    ...
    protected function prepare_nodes_for_apoa(): void {
        ...
        if ($this->page->context->contextlevel == CONTEXT_COURSECAT) {
            ...
            if ($this->page->context->depth > 1) {
                $this->remove('mycourses');
                $this->remove('courses');
                $this->remove_duplicate_items();
                //$this->remove($this->page->context->instanceid, \breadcrumb_navigation_node::TYPE_CATEGORY);
            }
            else {
                $this->clear_items();
            }
    ...
    /**
     * Get all the apoanavbaritem elements.
     *
     * @return apoanavbaritem[] apoa navbar items.
     */
    ...
    /**
     * Removes all navigation items out of this apoa navbar
     */
    ...
    /**
     * Remove a apoanavbaritem from the apoa navbar.
     *
     * @param  string|int $itemkey An identifier for the apoanavbaritem
     * @param  int|null $itemtype An additional type identifier for the apoanavbaritem (optional)
     */
    ...
    /**
     * Removes the action from the last item of the apoanavbaritem.
     */
