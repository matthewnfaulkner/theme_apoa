 ...
 */
namespace theme_apoa\output\format_tiles;
 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_tiles_course_output implements \renderable, \templatable {
    ...
    private function append_multi_section_page_data($data) {
        ...
        foreach ($secsall as $sectionnum => $section) {
            ...
            if ($sectionnum != 0 && $showsection) {
                ...
                if(isset($this->modinfo->sections[$section->section]) 
                    && count($this->modinfo->sections[$section->section]) == 1 ) {
                        $singleactivity = reset($this->modinfo->sections[$section->section]);
                        $cminfo = $this->modinfo->cms[$singleactivity];
